...
export default function ProblemCard({pb, onUpdate, activeTab}){
        ...
        if (editedCode.trim() !== '') {
            ...
            try {
                const res = await axios.post(
                    `${import.meta.env.VITE_API_URL}/problem/update`,
                    updatedProblem, 
    ...
    const onMarkDone = async(title) => {
        try {
            ...
            const res = await axios.post(`${import.meta.env.VITE_API_URL}/problem/done/${encodeTitle}`,{}, {
                headers:{
    ...
    const addToDueToday = async(title) => {
        try {
            const encodeTitle = encodeURIComponent(title);
            const res = await axios.post(`${import.meta.env.VITE_API_URL}/problem/addToDueToday/${encodeTitle}`,{}, {
                headers:{
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const resetProblem = async(title) => {
        try {
            const encodeTitle = encodeURIComponent(title);
            const res = await axios.post(`${import.meta.env.VITE_API_URL}/problem/reset/${encodeTitle}`,{}, {
                headers:{
                    Authorization: `Bearer ${token}`
                }
            });
