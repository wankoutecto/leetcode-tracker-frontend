        ...
        const fetchAllCategories = async () => {
            try {
                const [dueRes, futureRes, overdueRes, fullyRes, problemList] = await Promise.all([
                    axios.get(`${import.meta.env.VITE_API_URL}/problem/due/today`, {
                        headers: { Authorization: `Bearer ${token}` }
                    }),
                    axios.get(`${import.meta.env.VITE_API_URL}/problem/upcoming`, {
                        headers: { Authorization: `Bearer ${token}` }
                    }),
                    axios.get(`${import.meta.env.VITE_API_URL}/problem/overdue`, {
                        headers: { Authorization: `Bearer ${token}` }
                    }),
                    axios.get(`${import.meta.env.VITE_API_URL}/problem/completed`, {
                        headers: { Authorization: `Bearer ${token}` }
                    }),
                    axios.get(`${import.meta.env.VITE_API_URL}/problem/get/all`, {
                        headers: { Authorization: `Bearer ${token}` }
                    })
                ]);
